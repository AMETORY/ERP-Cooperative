...
func (s *SalesHandler) DownloadPdfHandler(c *gin.Context) {
	id := c.Param("id")
	...
	sales.Items = items
	pdf, err := s.orderSrv.SalesService.GetPdf(sales, "templates/pdf/invoice.html", "", "generated by jaraya", true, true)
	if err != nil {
		c.JSON(500, gin.H{"error": err.Error()})
		return
	}
	...
	c.Writer.Header().Add("Content-Disposition", "attachment; filename="+sales.Code+".pdf")
}
